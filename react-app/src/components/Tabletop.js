...
const Tabletop = ({ gameState }) => {
    ...
    return (
        <div className='page-container'>
            ...
            <div className='player-card-counters'>
                <div>{player1.username} Card Count: {gameState.player1Cards.length}</div>
                <div>Computer Card Count: {gameState.player2Cards.length}</div>
            </div>
        </div>
    );
};
